...
const Category = ({ category }) => {
    ...
    const submit = (e) => {
        e.preventDefault();
        patch(route("categories.update", category.id), {onSuccess: ()=> {setEditing(false); alert()}})
    };
            ...
            <div className="flex-1">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <small className="ml-2 text-sm text-gray-600">{dayjs(category.created_at).fromNow()}</small>{category.created_at !== category.updated_at && (<small className="text-sm text-gray-600">&middot; edited</small>)}
                        {category.created_at !== category.updated_at && <small className="text-sm text-gray-600"></small>}
                    </div>
                    {
